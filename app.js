...
async function extractThumbs(file) {
  ...
  // Clear title field so user enters their own name
...
async function publishVideo(e) {
  ...
  if (cloudOn() && storageOn() && uploadFile) {
    try {
      ...
      if (!cloudUrl) throw new Error('Cloudinary вернул пустой URL');

      const thumbUrl = await uploadThumbCloud(uploadThumbs[selectedThumb]);

      newV.src   = cloudUrl;
      newV.thumb = thumbUrl || newV.thumb;
      newV.id    = String(newV.id);

      await saveVideoCloud(newV);
      showToast('Видео сохранено в облако ☁️');
    } catch(err) {
      console.warn('Cloud upload failed, video kept locally:', err);
      showToast('Облако недоступно — видео сохранено локально');
    }
  }
}
...
function listenVideosCloud() {
  ...
  _videosUnsub = _fbDB.collection('videos')
    ...
    .onSnapshot(snap => {
      // Use the embedded 'id' field (our timestamp) for dedup, not Firestore doc ID
      const cloudIds = new Set(snap.docs.map(d => String(d.data().id || d.id)));
      DB       = DB.filter(v => !cloudIds.has(String(v.id)));
      ShortsDB = ShortsDB.filter(v => !cloudIds.has(String(v.id)));
      snap.forEach(doc => {
        const data = doc.data();
        const d = { firestoreId: doc.id, ...data, id: data.id || doc.id };
        if (d.createdAt?.toMillis) d.createdAt = d.createdAt.toMillis();
        if (!d.src) return; // skip videos with no playable source
...
async function switchStreamSource(source) {
  ...
  try {
    if (source === 'camera') {
      ...
      try {
        _localStream = await navigator.mediaDevices.getDisplayMedia({ video: { frameRate: 30 }, audio: true });
      } catch {
        _localStream = await navigator.mediaDevices.getDisplayMedia({ video: { frameRate: 30 }, audio: false });
      }
    }
    if (source === 'screen')
  ...
  } catch(err) { showToast('Нет доступа к камере/экрану: ' + (err.message || err)); }
}
...
async function goLive(e) {
  ...
  if (source === 'camera' || source === 'screen') {
    try {
      if (source === 'camera') {
        // Попытка с микрофоном, fallback без
        try {
        ...
        try {
          media = await navigator.mediaDevices.getDisplayMedia({ video: { frameRate: 30 }, audio: true });
        } catch {
          media = await navigator.mediaDevices.getDisplayMedia({ video: { frameRate: 30 }, audio: false });
        }
      }
    } catch(err) { showToast('Нет доступа к камере/экрану: ' + (err.message || err)); return; }
  }
